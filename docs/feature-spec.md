...
**ファイル**: `js/tutorial.js`  
**起動**: 遊び方モーダル上部の「▶ インタラクティブチュートリアル」ボタン

### 概要
実際のパズル（id:101）をソルバーで解析し、5つの解法手筋をステップ形式で体験できるインタラクティブ教材。

### 実装方式
- `TangoSolver.computeSolveSteps()` でパズル101の全ステップを生成
- techId 1〜5 それぞれの最初のステップを1つずつ選出（計5ステップ）
- 各ステップで「そこに至るまでの盤面状態」を前ステップ分の適用で再現

### 表示内容（各ステップ）
| 要素 | 内容 |
|------|------|
| プログレスバー | 1/5 〜 5/5 |
| 解説テキスト | `step.reason`（日）/ `step.reasonEn`（英） |
| ミニグリッド | 6×6、fixedセル・制約マーカー付き |
| ハイライトセル | 緑のパルスアニメーション（`.tut-cell.target`） |
| 操作 | ハイライトをタップ → 正解が入って自動次ステップ。「次へ」でスキップ可 |

### i18n
- `LANG` グローバル変数（app.jsで定義）を参照して日英切替
- テックネーム翻訳: `TECH_NAMES.ja / .en` オブジェクト
